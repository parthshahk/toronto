...
import { appendText, ALIGN, VALIGN } from './text.js';
...
/* ============================================== the surveyed unit model == */
//
// WHY THIS EXISTS. appendGroundFloor() above treats a building as having ONE frontage with ONE
// door on it, and decides what that door looks like from the building's night-lighting profile.
// Both halves of that are wrong at street level. A block face on Queen West is eight or ten
// separate premises with eight or ten different fronts; a condo tower's profile says
// "residential" while its podium is a bank, a pharmacy and two restaurants. The result was a
// street where nothing was where it really is.
//
// data/toronto.json now carries 8,451 SURVEYED UNITS across 3,261 buildings — each with its OSM
// category and value, its name, its brand, its house number, and the footprint segment and
// parameter that say exactly where along the frontage it sits. This block turns one of those
// into geometry, and turns a synthesised one into geometry that is indistinguishable from it
// (CONTRACT Amendment 9: the seam between surveyed and invented must not be visible).
//
// DETERMINISM. Nothing here draws from a shared rng stream. Every appearance decision is hashed
// on the unit's own WORLD POSITION through fallbackRng(), so a unit looks the same whether its
// tile was built first or last, alone or with its neighbours resident — the tiling contract in
// tiles.js, which the profile-driven path predates.
...
const MAT_GLASS_BRIGHT = [0.140, 0.140, 0.132, 0.72, 0, 0.08, 0];  // convenience, pharmacy
...
const MAT_STONE_DARK = [0.058, 0.057, 0.060, 0, 0, 0.24, 0];
// Rolling shutter, closed, and the housing box an open one lives in.
const MAT_SHUTTER_BOX = [0.056, 0.057, 0.060, 0, 0, 0.40, 0];
// A vacant unit's leasing board.
const MAT_BOARD = [0.150, 0.150, 0.146, 0, 0, 0.90, 0];
