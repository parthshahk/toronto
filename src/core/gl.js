// src/core/gl.js — WebGL2 plumbing: context creation, a Shader wrapper with
// type-dispatched uniform setting, static/dynamic interleaved triangle meshes, and a fast
// zero-allocation MeshBuilder.
//
// THE VERTEX FORMAT IS NOT DEFINED HERE. It lives in core/vertex.js — one file, per CONTRACT
// Amendment 11.2 — and this module imports it like everybody else. Nothing below may hard-code a
// stride, a float count or a byte offset into a vertex.
//
// Zero dependencies.

import {
  VERT_FLOATS, ATTR, VERT_OFFSET, VERT_SIZE, VERT_STRIDE, packUv,
} from './vertex.js';
...
function bindVertexFormat(gl) {
  // Layout comes from core/vertex.js: float offsets become byte offsets, nothing is spelled out
  // twice. Order matters only for readability — each pointer is fully self-describing.
  const names = ['pos', 'normal', 'color', 'mat', 'uv'];
  for (let i = 0; i < names.length; i++) {
    const k = names[i];
    gl.enableVertexAttribArray(ATTR[k]);
    gl.vertexAttribPointer(ATTR[k], VERT_SIZE[k], gl.FLOAT, false, STRIDE, VERT_OFFSET[k] * 4);
  }
}
...
export class MeshBuilder {
  ...
  append(other, ox = 0, oy = 0, oz = 0, yaw = 0, scale = 1) {
    ...
    for (let i = 0; i < n; i += VERT_FLOATS) {
      ...
      dst[k + 4] = ny;
      dst[k + 5] = -nx * s + nz * c;
      // Everything from aColor on is position-independent, so it copies verbatim. Driven off
      // VERT_OFFSET/VERT_FLOATS so a new channel is carried without touching this line.
      for (let f = VERT_OFFSET.color; f < VERT_FLOATS; f++) dst[k + f] = src[i + f];
      k += VERT_FLOATS;
    }
    this._n = k;
    return this;
